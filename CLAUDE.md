...
>
> **ALWAYS update them when anything relevant changes.** If you change colors, layouts, or UI patterns → update `UX.md`. If you change lore, faction names, location names, or world tone → update `LORE.md`. These files are the source of truth. Do not let them go stale.
>
> **SPEC.md** is fully auto-generated by `scripts/build-spec.sh` (7 sections: Vision → Setting → Mechanics → UX → Devlog → Plan → Data). Authored content for SPEC lives in `scripts/templates/` — edit those, not SPEC.md directly.
>
...
- **`engine/hud.js`** — thin orchestrator; bottom strip is DOM (`#hud-bottom`); canvas sub-renderers in `engine/hud/`. See `@UX.md` for layout details.
...
- **UI overlays** — narrative panel (`#narrative-panel`, right 30%), ship panel (`#ship-panel`, left 30%), bottom HUD (`#hud-bottom`, 48px fixed bar). Docking sets `isDocked = true`, skipping the sim loop. Both panels use `stopPropagation` to prevent canvas input bleed. See `@UX.md` for visual layout details.
