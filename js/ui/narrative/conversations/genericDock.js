...
async function _repairArmor(game, station, log) {
  ...
  if (game.scrap < cost) {
    log.dialogue('Mechanic', `That'll be ${cost} scrap. You don't have enough.`);
    return;
  }

  log.dialogue('Mechanic', `Armor's banged up. ${cost} scrap to patch it all.`);
...
async function _repairHull(game, station, log) {
  ...
  if (game.scrap < cost) {
    log.dialogue('Mechanic', `Hull work's ${cost} scrap. Come back when you have it.`);
    return;
  }

  log.dialogue('Mechanic', `Hull integrity's low. ${cost} scrap for a full restore.`);
...
async function _refuel(game, station, log) {
  ...
  if (game.scrap < cost) {
    log.dialogue('Fuel Tech', `Full tank's ${cost} scrap. You're short.`);
    return;
  }

  log.dialogue('Fuel Tech', `Tank's at ${Math.floor(game.fuel)} out of ${game.fuelMax}. Full fill: ${cost} scrap.`);
...
async function _reactor(game, station, log) {
  ...
  for (const mod of fissionMods) {
    ...
    if (mod.isOverdue) {
      log.dialogue('Engineer', `Your ${mod.displayName} is overdue. Badly. ${mod.overhaulCost} scrap for a full overhaul.`);
    } else {
      const remaining = mod.overhaulInterval - mod.timeSinceOverhaul;
      const h = Math.floor(remaining / 3600);
      const m = Math.floor((remaining % 3600) / 60);
      log.dialogue('Engineer', `${mod.displayName} — next overhaul in ${h}h ${m}m. ${mod.overhaulCost} scrap if you want it done now.`);
    }
