...
export async function ashveilHub(ctx) {
  ...
  if (!game.storyFlags?.visited_ashveil) {
    log.narrate("The Anchorage fills your viewport — a decommissioned colony ship, repurposed into something between a town and a machine. Hull sections jut at odd angles, connected by struts and pressurized walkways. Docked ships cling to the outer piers like barnacles.", 'flavor');
    log.narrate("Inside, the air smells of recycled atmosphere and welding flux. Light filters through patched viewports in bands of amber and white.", 'flavor');
    if (game.storyFlags) {
  ...
  log.dialogue('Harbor Control', "Anchorage Control, you're cleared to dock. Mind the gantry — it's on a timer.", '#00ffcc');
