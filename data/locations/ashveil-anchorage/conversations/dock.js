...
export async function ashveilDock(ctx) {
  ...
  log.setNpcContext({
    maro: { name: 'Chief Maro', color: '#44aaff' },
  });
  ...
  if (!game.storyFlags?.met_ashveil_chief) {
    log.dln("You need work done, you talk to me. Nobody else touches your ship without my say.", 'maro');
    if (game.storyFlags) game.storyFlags.met_ashveil_chief = true;
  } else {
    log.dln("What have you done to it this time?", 'maro');
  }
  ...
  while (true) {
    ...
    if (!needsArmor && player.hullCurrent >= player.hullMax && fuelNeeded <= 0.5) {
      log.dln("Ship's in good shape. Fuel's topped off. You don't need me.", 'maro');
    }
    ...
    switch (action.type) {
      case 'armor': {
        ...
        log.seq([
          "maro::Plating's set. Good as it gets without a proper shipyard.",
      ...
      case 'hull': {
        ...
        log.seq([
          "maro::Structural integrity's back to spec. Try not to fly into anything.",
          `s::-${action.cost} scrap. Hull restored.`,
        ]);
        break;
      }
      ...
      case 'fuel': {
        ...
        log.seq([
          "maro::Tanks are full. Safe travels.",
      ...
      case 'reactor': {
        ...
        mod.resetOverhaul();
        if (mod.isOverdue) {
          log.dln(`That ${mod.displayName} was running hot. Another week and it would've been a paperweight.`, 'maro');
        } else {
          log.dln(`${mod.displayName} is serviced. Clock's reset.`, 'maro');
        }
...
registerContent('conversations', 'ashveilDock', ashveilDock);
